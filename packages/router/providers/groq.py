...
`https://api.groq.com/openai/v1/chat/completions`, so this is structurally
identical to the OpenAI provider with a different base URL.
"""
...
class GroqProvider(BaseProvider):
    ...
    async def chat(
        ...
        if not self.api_key:
            raise ProviderError(self.name, "GROQ_API_KEY is missing", status_code=503)
